...
During the v0.8.3 development milestone the root `go.mod` and every in-tree plugin submodule pin a pseudo-version of `cyoda-go-spi` (currently `v0.8.3-0.20260726155628-023322e2edc0`) while the `ScheduledTaskStore` surface, the arm-time Function additions (`TransitionSchedule.Function`, `ReconcileRequest.Cancel`), the Cloud-aligned type-directed search kernel (`DataType`/`Decimal`/`IsAssignableTo`, `ExpandLeaf`/`EvalLeaf`), and the bounded-or-fail direct-search change (`Searcher.Search`, `MergePage` → `MergeBounded`, `SearchOptions.Offset` removed — see `docs/cloud-parity/direct-search-bounded-or-fail.md`) land. The pseudo-version pin advances as milestone work merges; it is not re-pinned per PR, only when a dependent change needs the newer SPI surface to build. The final pin bump happens at end of milestone, per the coordinated-release procedure in [`MAINTAINING.md`](./MAINTAINING.md), to the real **`cyoda-go-spi v0.8.3`** tag. That tag is breaking despite being a patch — pre-1.0 this module ships breaking changes on patches, and the `### Breaking` changelog section rather than the version component is what flags it.
