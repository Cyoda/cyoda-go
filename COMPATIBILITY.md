...
|---|---|---|---|
| **`v0.8.4`** _(planned)_ | `cyoda-go-spi v0.8.3` | `cyoda-go-spi v0.8.3` | — (no SPI change, deliberately). The transaction-lifecycle work adds `503 STORAGE_UNAVAILABLE`, which a storage backend opts into by returning an error satisfying `interface{ StorageUnavailable() bool }`, matched with `errors.As` — a structural marker, so it needs no SPI declaration. Hence no SPI tag and no coordinated cross-repo release: the commercial backend keeps working unchanged on `v0.8.3`, and can adopt the marker whenever it chooses to turn its pool-exhaustion failures into a retryable 503 instead of a generic 500. Chart `version:` / `appVersion:` and out-of-tree pin guidance are unchanged too. The version component stays a patch — the HTTP/wire API is unchanged in shape; what changed is which statuses are reachable and declared. Read [`CHANGELOG.md`](./CHANGELOG.md)'s `### Breaking` section before upgrading: three `CYODA_TX_*` variables are removed, and three PostgreSQL ceilings that did not exist now apply by default. |
