...
During the v0.8.3 development milestone the root `go.mod` and every in-tree plugin submodule pin a pseudo-version of `cyoda-go-spi` (currently `v0.8.3-0.20260725045352-61d665caf0c6`) while the `ScheduledTaskStore` surface, the arm-time Function additions (`TransitionSchedule.Function`, `ReconcileRequest.Cancel`), and the Cloud-aligned type-directed search kernel (`DataType`/`Decimal`/`IsAssignableTo`, `ExpandLeaf`/`EvalLeaf`) land. The pseudo-version pin advances as milestone work merges; it is not re-pinned per PR, only when a dependent change needs the newer SPI surface to build. The final pin bump to the real `cyoda-go-spi v0.8.3` tag happens at end of milestone, per the coordinated-release procedure in [`MAINTAINING.md`](./MAINTAINING.md).
