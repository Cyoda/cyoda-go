...
// an id already written in this transaction conflicts — as postgres answers.
func TestTx_SaveThenCompareAndSave_Conflicts(t *testing.T) {
	...
	_, err = store.CompareAndSave(txCtx, &spi.Entity{Meta: committed.Meta, Data: []byte(`{"n":3}`)}, committed.Meta.TransactionID)
	if !errors.Is(err, spi.ErrConflict) {
