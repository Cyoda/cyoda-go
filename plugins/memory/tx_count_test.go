...
func TestTxCount_EveryBufferShape(t *testing.T) {
	...
	seedStates(t, store, ctx, ref, "open", "closed", "open", "closed") // e00 open, e01 closed, e02 open, e03 closed

	txID, txCtx, _ := tm.Begin(ctx)
	defer func() { _ = tm.Rollback(txCtx, txID) }()
	save := func(id, state string) {
		t.Helper()
		if _, err := store.Save(txCtx, &spi.Entity{Meta: spi.EntityMeta{ID: id, TenantID: "tenant-it", ModelRef: ref, State: state}, Data: []byte(`{}`)}); err != nil {
