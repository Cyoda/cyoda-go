...
// ErrTxAlreadyCommitted, the same guard the read paths already carry (see
// TestTx_ClosedTransaction_RefusesEveryEntryPoint). Without it a caller
// holding a stale transaction context silently buffers a write into a
// transaction that has already been flushed and closed — the write is then
// never applied anywhere, a silent data loss (see
// .claude/rules/correctness-over-availability.md).
func TestTx_ClosedTransaction_RefusesEveryWrite(t *testing.T) {
	f, tm := newTxManager(t)
	ctx := tenantCtx(spi.TenantID("tenant-closed-write"))
