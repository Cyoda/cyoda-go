...
// TestSqliteGroupedAggregate_DeclinesOnSoundSupersetComparisonFilter pins the
// SQL-pushdown soundness contract (query_planner.go leafExact) at the
...
// stored value a sub-ULP beyond the operand can collide to the same REAL, so
// leafExact(Gt) is false and planQuery installs the FULL filter as postFilter
// even though the leaf itself pushed cleanly. A native SQL GROUP BY cannot
// safely apply that residual after aggregating (it would corrupt per-bucket
// counts), so GroupedAggregate MUST decline with ErrAggregationNotPushdownable
// — exactly like the already-pinned fully non-pushable regex case
// (TestSqliteGroupedAggregate_DeclinesOnResidualFilter) — even though Gt DOES
// translate to SQL, unlike regex which never does. Contrast with
// TestSqliteGroupedAggregate_UsesNativeGroupByOnIsNullOnlyFilter below (an
// EXACT leaf, which DOES use the native fast path) and with
// TestSqliteGroupedAggregate_PushesCountByState above (no condition at all).
func TestSqliteGroupedAggregate_DeclinesOnSoundSupersetComparisonFilter(t *testing.T) {
	_, store, ctx := gsNewStore(t)
...
// TestSqliteGroupedAggregate_UsesNativeGroupByOnIsNullOnlyFilter contrasts
// the test above: IsNull IS leafExact (the only exact ops are IsNull/NotNull
// — see leafExact), so a plan carrying only an IsNull leaf is fully EXACT and
// the native SQL GROUP BY fast path IS used (no decline, no residual
// re-check needed).
func TestSqliteGroupedAggregate_UsesNativeGroupByOnIsNullOnlyFilter(t *testing.T) {
	_, store, ctx := gsNewStore(t)
	gsSave(t, ctx, store, "a", "available", map[string]any{"price": 100.0})
...
// TestSqliteGroupedAggregate_StreamingFallbackCorrectAtSoundSupersetBoundary
