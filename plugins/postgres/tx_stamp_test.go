...
//
// Backends diverged here. This one stamped at write time, but only when the
// caller left the field empty, so a caller-supplied value survived and became
// the compare-and-save precondition; memory and sqlite stamped only at
// commit. Callers in this repo all stamp their own transaction's ID, which is
// why it stayed latent.
func TestTx_UnstampedWrite_CarriesTheTransactionID(t *testing.T) {
	f, tm := newAttrFactory(t)
	ctx := ctxWithTenant(spi.TenantID("tenant-stamp"))
...
func TestTx_ForeignStampIsOverwritten(t *testing.T) {
	f, tm := newAttrFactory(t)
	ctx := ctxWithTenant(spi.TenantID("tenant-stamp2"))
