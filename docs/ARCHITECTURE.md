...
**Panic containment.** Panics are recovered at the gRPC server (unary and stream interceptors), on every HTTP route, and in the async-search goroutine. A recovered panic on either request door logs the value and stack under a ticket UUID, returns a sanitized error carrying that ticket, and marks the node unhealthy. Nothing resets that flag: `GET /health` on the API listener reports `503 DOWN` from then on, and the admin listener's `/readyz` (§7.5) reports `503` for the same reason — so the node is removed from its Service endpoints and stops receiving traffic. A node that has panicked has unverified state, so taking it out of service is the correct response rather than continuing to serve from a state nothing has checked. `/livez` is unconditional and does not read the flag: the node is drained, not restarted, because a deterministic panic would otherwise become a restart loop. `/readyz` fails for two independent reasons — storage not initialised, or a recovered panic — and reports which in the server-side log while answering the probe generically.
