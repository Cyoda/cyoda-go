...
**Panic containment.** Panics are recovered at the gRPC server (unary and stream interceptors), on every HTTP route, and in the async-search goroutine. A recovered panic on either request door logs the value and stack under a ticket UUID, returns a sanitized error carrying that ticket, and marks the node unhealthy: `GET /health` on the API listener then reports `503 DOWN` permanently. A node that has panicked has unknown state, so taking it out of service is the correct response rather than continuing to serve from a state nothing has verified. The admin listener's `/livez` and `/readyz` (§7.5) do not read that flag — `/livez` is unconditional and `/readyz` reports storage initialisation.
