...
The path fails closed. A resolved expiry at or before the resolved fire time is
**born expired**: the transition is never armed, any prior scheduling is
cancelled, and a `SCHEDULED_TRANSITION_EXPIRE` audit event is recorded — the
entity write still succeeds. A malformed or wrong-kind result rejects the write
with the new `500 SCHEDULE_FUNCTION_INVALID_RESULT`. A compute node that is
unreachable, disconnected, or timing out surfaces a retryable `503`; there is no
silent skip.
