...
| D21 | Postgres `cyoda_try_float8(text)` ships as `LANGUAGE plpgsql IMMUTABLE PARALLEL SAFE` with a **regex pre-filter** as the fast path and a narrow `EXCEPTION` block as the safety net. Regex uses `\A` / `\Z` string-anchors (not `^` / `$`) so trailing newline / carriage-return doesn't sneak past. The fast path (regex matches AND cast succeeds AND result is finite) takes no subtransaction. The `EXCEPTION WHEN numeric_value_out_of_range OR invalid_text_representation` block catches the edge case where the regex passed but the literal overflows float8 (e.g. `'1e500'`) — discovered during Task 13 implementation: postgres's `text::float8` cast raises `22003` on overflow, not Infinity, so the v3 spec's claim that "NULLIF strips Infinity" was factually wrong. | The original "no EXCEPTION" rationale was about avoiding per-row subtransaction cost on valid inputs (~30M subtransactions on a 10M-row × 3-aggregation scan). With the regex pre-filter, EXCEPTION only fires on regex-passes-but-cast-overflows — a rare path for clean data. Clean rows still go pure-SQL fast. Documented per Gate 6. |
