...
> ## STATUS 2026-09-03 — Tasks 1-7 are DONE. The plan is executed.
>
> Every task in this plan is implemented and committed on `fix/477-search-fallback`
> (pushed). `make test-full` and `make race` are green; `go vet` is clean on the
> root module and all three plugin submodules; the oasdiff gate passes. The
> exit checks in Task 7 Step 4 return nothing but ModelStore's own `GetAll` and
> historical prose.
>
> Beyond the plan, on the same branch: the two findings the earlier banner
> listed are both acted on. `fire_scheduled.go` refuses to fire an entity with
> no committed transaction ID, before running the transition (`fdbb865`) — the
> segmented case had been landing with no precondition checked at all. And all
> three backends now stamp `Meta.TransactionID` with the transaction's own ID
> at write time (`c621fd2`), which closes the memory/sqlite-vs-postgres
> divergence; the binding `spitest` case for that rule needs the next SPI wave
> and is recorded as an obligation in `COMPATIBILITY.md`.
>
> What remains is the PR itself and the issue bookkeeping in Task 7 Step 6.
...
- [x] **Step 1: Replace each assertion**
...
- [x] **Step 2: Run the package tests that already exist for delete**
...
- [x] **Step 1: The failing test is `TestErrCode_Parity`**
...
- [x] **Step 2: Retire the code everywhere**
...
- [x] **Step 2: Migrate the mechanical sites**
...
- [x] **Step 3: The two non-mechanical sites**
...
- [x] **Step 4: Delete the fallback tests and doubles; update `failingEntityStore`**
...
- [x] **Step 5: Run the iteration tier**
...
- [x] **Step 2: Cloud-parity note and index**
...
- [x] **Step 3: CHANGELOG and COMPATIBILITY**
...
- [x] **Step 4: Repin, full suite, race**
...
- [x] **Step 6: Review gates, PR, issue bookkeeping**
