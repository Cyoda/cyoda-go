...
| F6 | No in-process transaction reaper exists or is needed: every write releases its transaction via a deferred scope on every exit path, including a panic, backstopped by PostgreSQL `statement_timeout`/`idle_in_transaction_session_timeout`. If a transaction still looks stuck, check the pool's connection count (`pgxpool.Stat()`) and confirm `idle_in_transaction_session_timeout` is set on the connection; a node that has recovered a panic reports `503 DOWN` and is expected to be restarted by its supervisor, not self-healed by a reaper | `internal/domain/entity/service.go` (`defer scope.Release()`); `plugins/postgres/config.go` (ceiling defaults); `internal/api/health.go` (`503 DOWN` on recovered panic) | Regressed if the deferred release is removed from a write path, a ceiling is left unset, or a recovered panic no longer marks the node unhealthy |
