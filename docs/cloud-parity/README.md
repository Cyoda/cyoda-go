...
| `search-has-one-path.md` | A synchronous search reaches storage through exactly one call, `EntityStore.Search`; the whole-model read and the in-memory fallback are gone. A condition that cannot be translated is `400 INVALID_CONDITION` (`INVALID_FIELD_PATH` when path-shaped) rather than a request served by a different mechanism, and a nil condition is refused rather than read as match-all. Async submit refuses at submission what its executor could not run |
