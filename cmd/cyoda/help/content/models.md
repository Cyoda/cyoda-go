...
**A value's kind must be one the field declares.** A field declared as a scalar accepts a scalar; declared as an array, an array; declared as an object, an object. A value of any other kind is rejected with `400 BAD_REQUEST` (`expected scalar, got array`), independently of the declared `changeLevel`. `null` is always accepted — it is the absence of a value, not a kind. A field observed in more than one kind — through repeated imports while `UNLOCKED` — declares each of them, and each stays admissible; the export names every branch.
