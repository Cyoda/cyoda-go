...
**A value costs a `TYPE`-level change only if the field cannot already hold its type.** Numeric types widen, and a field admits everything that widens into what it declares. A whole number in the `INTEGER` range (|v| ≤ 2³¹−1) written to a field declared `DOUBLE` is the everyday case — the classifier reads `1000`, `1000.0` and `1e3` alike as an integer, and `INTEGER` widens into `DOUBLE` — so the write proposes no schema change and is accepted at every `changeLevel`, `ARRAY_LENGTH` included.

The envelope is what the widening lattice allows, not "whole numbers". A larger whole number classifies `LONG` or wider, and `LONG → DOUBLE` is deliberately not a widening — 2⁶³ exceeds `DOUBLE`'s 53-bit mantissa, so the conversion loses precision. Writing `2147483648` to a `DOUBLE` field is therefore a real type change: refused below `TYPE`, and at `TYPE` it widens the field to `UNBOUND_DECIMAL`, the narrowest type that holds both. A decimal into a field declared `INTEGER` is a type change for the same reason.
