...
**An unsatisfiable comparison follows operator polarity.** When an operand cannot be satisfied by a stored value's own declared type family — the operand parses into no type that family declares, or the field declares no type at all — a **positive** operator (`EQUALS`, `GREATER_THAN`, the string operators, …) answers non-match, and a **negative** operator (`NOT_EQUAL`, `NOT_CONTAINS`, `INOT_*`, …) answers match instead, per stored-value type family. `$.n NOT_EQUAL 12.5` on an `INTEGER` field matches every entity holding a number there, because no integer equals `12.5` — the same answer PostgreSQL gives for `5::int <> 12.5`. This is a determinate answer about the entity, not a rejection, and null/absent values are unaffected (see NULL SEMANTICS below).
