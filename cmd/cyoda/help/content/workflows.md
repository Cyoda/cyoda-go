...
`simple` and `array` criteria address entity data fields by `jsonPath`, under the same grammar a search condition obeys — the `$.` leader is required, and a path outside it is rejected at **import** with `400 VALIDATION_FAILED` rather than at every later evaluation. Write `$.amount`, not `amount`. Array subscripts (`$.tags[*].name`, `$.arr[0]`) are valid: criteria are evaluated in memory, which resolves them. See `cyoda help search` for the grammar.
