...
- A callback never commits the transaction it joined — only the operation that began it does. A callback write whose workflow reaches a `COMMIT_BEFORE_DISPATCH` processor is refused with `409` `errors.COMMIT_IN_JOINED_TRANSACTION` at that processor: it is not dispatched, and the joined transaction is neither flushed nor committed. What the workflow did before it stays in the joined transaction, for the calling operation to keep or discard. Not retryable: change the processor's `executionMode`, or make the write outside the callback, without the token.
