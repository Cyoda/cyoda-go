...
- Keep a callback's **answer** under 10 MiB as well. The answer is held in memory for the same reason the request is, and under the same ceiling: an answer that would pass it fails the callback with a `500` and a ticket rather than being cut short, on either door. Page a large read — `pageSize` and `pageNumber` on a get-all or a search — instead of asking for everything in one callback.
