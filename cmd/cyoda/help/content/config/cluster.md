...
- `CYODA_NODE_ID` (string, default: unset) — unique node identifier; required when `CYODA_CLUSTER_ENABLED=true`; any non-empty string is accepted. It must be distinct on every node: a hand-over between nodes is sealed for the node it is sent to, by this id, so two nodes sharing one id can open each other's (see the `cluster` help topic, `DISPATCH REPLAY PROTECTION`). A node that finds its id already held by a live node of the cluster keeps trying for the whole of `CYODA_STARTUP_TIMEOUT` and refuses to start if the id is still held when that budget runs out, naming this setting, the address the id was found at and the seed it was learned from; the node already holding it keeps serving. The retrying is what a crash needs: a node that did not leave gracefully and comes back at another address finds its peers still holding the record of its previous life, and nothing distinguishes that record from another node's — so the first attempts are refused, and the node starts on the attempt made after its peers have reaped the record, usually a few seconds later. The check runs only while a node is starting, and it runs there twice over: on the join exchange with a seed, and on the membership gossip that follows it, which is what two nodes starting at the same instant have instead of an exchange. It does **not** run afterwards. Once a node is serving, an id claimed from a second address is logged at ERROR, once per address, and nothing else happens: both nodes keep serving under the one id, and each can open the other's hand-overs. Two clusters that each started healthily under overlapping ids and only later meet — a network policy opened, a namespace split joined up, a partition healed — are exactly that case, and so is a node started with no seeds that is later given some. Watch for the ERROR line, fix the id and restart the node that has the wrong one. Together with `CYODA_NODE_ADDR` and `CYODA_GRPC_NODE_ADDR` it must fit the 512-byte gossip metadata (about 420 bytes for the three values together; 89 bytes are framing and the longest list version); a node whose identity does not fit refuses to start.
