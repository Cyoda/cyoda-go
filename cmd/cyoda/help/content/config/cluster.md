...
- `CYODA_NODE_ID` (string, default: unset) — unique node identifier; required when `CYODA_CLUSTER_ENABLED=true`; any non-empty string is accepted. It must be distinct on every node: a hand-over between nodes is sealed for the node it is sent to, by this id, so two nodes sharing one id can open each other's (see the `cluster` help topic, `DISPATCH REPLAY PROTECTION`). A node that finds its id already held by a live node of the cluster refuses to start, naming this setting, the address the id was found at and the seed it was learned from; the node already holding it keeps serving. The same refusal follows a crash: a node that did not leave gracefully and comes back at another address finds its peers still holding the record of its previous life, and nothing distinguishes that record from another node's, so it is refused too — it starts once its peers have reaped the stale record. That check runs during the join exchange with a seed, so it is a strong detector rather than a proof: it cannot fire when the seed does not yet know the node holding the id — two nodes starting at the same instant, a partition healing — or when no seeds are configured. Any node that then sees one id claimed from two addresses logs it at ERROR, once per address, and keeps serving. Together with `CYODA_NODE_ADDR` and `CYODA_GRPC_NODE_ADDR` it must fit the 512-byte gossip metadata (about 420 bytes for the three values together; 89 bytes are framing and the longest list version); a node whose identity does not fit refuses to start.
