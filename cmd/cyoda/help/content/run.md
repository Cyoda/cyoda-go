...
- `GET /livez` — liveness probe. Returns `200 OK` with body `ok` when the admin server is accepting connections. No business logic check is performed, so a node that has recovered a panic still passes it: such a node is drained, not restarted.
- `GET /readyz` — readiness probe. Returns `200 OK` with body `ready` when the server has completed startup and is ready to serve requests. Returns `503` while the storage backend is initializing or migrations are pending, and permanently once the node has recovered a panic — its state is then unverified, so it stops receiving traffic. Which of the two fired is in the server log; the response body stays generic.
