...
Every workflow definition must declare a schema version in `MAJOR.MINOR` format (for example `"version": "1.0"`). This error is returned when the import request contains a version string that does not match any supported schema version.

Common causes:

- The `"version"` field is a bare integer (e.g. `"1"`) instead of `MAJOR.MINOR` (e.g. `"1.0"`).
- The `"version"` field refers to a future or deprecated schema revision not supported by this server build.
