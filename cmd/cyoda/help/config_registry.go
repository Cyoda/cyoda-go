...
var rootConfigVars = []ConfigVar{
	...
	{Name: "CYODA_BOOTSTRAP_USER_ID", Topic: "auth", Type: "string", Default: "admin", Description: "User ID for the bootstrap client; valid UTF-8, 1 to 255 characters with no control character, noncharacter or U+FFFD, not beginning with the reserved word oidc: — in jwt mode with a bootstrap client configured, a value outside it fails startup."},
