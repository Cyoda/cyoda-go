...
func (d *ProcessorDispatcher) DispatchCriteria(ctx context.Context, entity *spi.Entity, criterion json.RawMessage, target string, workflowName string, transitionName string, processorName string, txID string) (bool, string, error) {
	...
	member := d.registry.FindByTags(tenantID, fn.Config.CalculationNodesTags)
	if member == nil {
	...
	resp, err := d.dispatchCalloutToMember(ctx, member, EntityCriteriaCalculationRequest, req, requestID, txID, fn.Config.ResponseTimeoutMs, "criteria", fn.Name)
	if err != nil {
		return false, "", err
	}
	if resp.Matches != nil {
