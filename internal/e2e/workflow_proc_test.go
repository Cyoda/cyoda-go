...
func TestWorkflowProc_Loopback(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "loopback-wf", "initialState": "NONE", "active": true,
			"states": {
...
func TestWorkflowProc_CascadeDepthLimit(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "loop-wf", "initialState": "A", "active": true,
			"states": {
...
func TestWorkflowProc_ProcessorModifiesData(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "modify-wf", "initialState": "NONE", "active": true,
			"states": {
				"NONE": {"transitions": [{"name": "init", "next": "CREATED", "manual": false,
...
func TestWorkflowProc_MultipleProcessorsSameTransition(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "multi-proc-wf", "initialState": "NONE", "active": true,
			"states": {
				"NONE": {"transitions": [{"name": "init", "next": "CREATED", "manual": false,
					"processors": [
...
func TestWorkflowProc_FullAuditTrail(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "audit-trail-wf", "initialState": "NONE", "active": true,
			"states": {
				"NONE": {"transitions": [{"name": "init", "next": "CREATED", "manual": false,
...
func TestWorkflowProc_UpdateWithCBD_DurablyCommitsPostCascadeState(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "cbd-update-wf", "initialState": "NONE", "active": true,
			"states": {
...
func TestWorkflowProc_UpdateWithCBD_StaleIfMatchAbortsBeforeDispatch(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "cbd-ifmatch-stale-wf", "initialState": "NONE", "active": true,
			"states": {
				"NONE":     {"transitions": [{"name": "init", "next": "PENDING", "manual": false}]},
				"PENDING":  {"transitions": [{"name": "approve", "next": "APPROVED", "manual": true,
...
func TestWorkflowProc_PostTxIdMatchesAuditEndpoint(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "txid-wf", "initialState": "NONE", "active": true,
			"states": {
				"NONE": {"transitions": [{"name": "init", "next": "CREATED", "manual": false}]},
				"CREATED": {"transitions": [{"name": "finish", "next": "DONE", "manual": false}]},
				"DONE": {}
...
func TestWorkflowProc_CreateWithCBD_DurablyCommitsPostCascadeState(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "cbd-create-wf", "initialState": "NONE", "active": true,
			"states": {
				"NONE":     {"transitions": [{"name": "init", "next": "PENDING", "manual": false}]},
...
func TestWorkflowProc_SearchSeesPreCalloutStateDuringDispatch(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "cbd-vis-wf", "initialState": "NONE", "active": true,
			"states": {
				"NONE":     {"transitions": [{"name": "init", "next": "PENDING", "manual": false}]},
				"PENDING":  {"transitions": [{"name": "approve", "next": "APPROVED", "manual": true,
...
func TestWorkflowProc_UpdateWithoutCBD_RegressionBound(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "sync-regression-wf", "initialState": "NONE", "active": true,
			"states": {
				"NONE":     {"transitions": [{"name": "init", "next": "PENDING", "manual": false}]},
				"PENDING":  {"transitions": [{"name": "approve", "next": "APPROVED", "manual": true,
...
func TestWorkflowProc_LoopbackWithCBD(t *testing.T) {
	...
	wf := `{
		"importMode": "REPLACE",
		"workflows": [{
			"version": "1.1", "name": "cbd-loopback-wf", "initialState": "NONE", "active": true,
			"states": {
